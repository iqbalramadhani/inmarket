...
<div class="modal-body gry-bg px-3 pt-0">
    ...
    <div class="mt-2 col-12 d-flex justify-content-center">
        ...
        <div class="" id="button-proceed" style="margin-right: 5px;">
            @if($status !== 'on_delivery' && $status !== 'cancelled')
        ...
        <div class="{{(!$orderDetail->is_confirmed) ? 'd-none' : ''}} is_confirmed"  style="margin-right: 5px;">
            <a class="btn btn-warning" href="{{route('post-review.create', ['order_code' => $order->code])}}">Beri Ulasan</a>
        </div>
        @if($orderDetail->delivery_status=='on_delivery' || $orderDetail->delivery_status=='delivered')
            <!-- <span class="mx-2"></span> -->
            <a href="{{route('customers.track-order.index', ['order_code' => $order->code])}}" class="btn btn-outline-primary ">
                Lacak
            </a>
        @endif
    </div>
...
<script type="text/javascript">
    $('#buttonCancelOrder').on('click', function(){
        $.ajax({
            method: 'GET',
            url: "{{route('seller.cancel-order-modal', ['order_code' => $order->code])}}",
