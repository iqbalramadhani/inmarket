                ...
                <div class="shadow-sm bg-white p-3 p-lg-4 rounded text-left">
                    <div class="mb-4">
                        <div class="row gutters-5 d-none d-lg-flex border-bottom mb-3 pb-3">
                            <div class="col-md-5 fw-600 ml-0">{{ translate('Product')}}</div>
                        ...
                        <ul class="list-group list-group-flush">
                            ...
                            <form action="{{ route('checkout.shipping_info') }}" method="POST" enctype="multipart/form-data">
                                @csrf


                                @foreach($data as $data)
                                <input id="{{$data['shop']->id}}" checked class="checkAll" type="checkbox" onclick="checkAllItemBelow({{$data['shop']->id}})">
                                {{$data['shop']->name}}
                                ...
                                <li class="px-0 px-lg-3" style="position: relative; display: block; padding: 0.75rem 1.50rem; background-color: #fff; margin-left: 3px;">
                                    <div class="row gutters-5">
                                        <div>
                                            <input type="checkbox" checked class="{{$data['shop']->id}} checboxsingle" data-cardId="{{ $cartItem['id'] }}" onclick="checkSingle({{$data['shop']->id}})" name="selected_product[]" value="{{$cartItem['product_id']}}">
                                        </div>
                                        <div class="col-lg-5 d-flex" style="margin-right: -20px;">
                                        ...
                                        <div class="col-lg col-6 order-4 order-lg-0">
                                            ...
                                            <div class="row no-gutters align-items-center aiz-plus-minus mr-2 ml-0">
                                                ...
                                                <input type="number" name="quantity[{{ $cartItem['id'] }}]" class="col border-0 text-center flex-grow-1 fs-16 input-number" placeholder="1" value="{{ $cartItem['quantity'] }}" min="{{ $product->min_qty }}" max="{{ $product_stock->qty }}" data-price="{{ convert_price($cartItem['price']) }}" onchange="updateQuantity(this)" data-idcart="{{ $cartItem['id'] }}">
                                        ...
                                        <div class="col-lg col-4 order-3 order-lg-0 my-3 my-lg-0">
                                            ...
                                            <span class="fw-600 fs-16 text-primary totalPrice{{ $cartItem['id'] }}">{{ single_price(($cartItem['price'] + $cartItem['tax']) * $cartItem['quantity']) }}</span>
                                            <span class="d-none totalPriceGetting totalPricePlain{{ $cartItem['id'] }}">{{ convert_price(($cartItem['price'] + $cartItem['tax']) * $cartItem['quantity']) }}</span>
                                        </div>
                                ...
                                <div class="px-3 py-2 mb-4 border-top d-flex justify-content-between">
                                    ...
                                    <span class="fw-600 fs-17" id="subtotal">0</span>
                                </div>
...
<script type="text/javascript">
    ...
    function updateQuantity(data) {
        console.log(data);
        let total = $(data).data('price') * $(data).val()
        $('.totalPricePlain' + $(data).data('idcart')).html(total)
        $('.totalPrice' + $(data).data('idcart')).html(formatRupiah(total))
        totalin()
    }
    ...
    function totalin() {
        ...
        $('.checboxsingle').each(function() {
            if (this.checked) {
                total = total + parseInt($('.totalPricePlain' + $(this).data('cardid')).text())
            }
        })
    ...
    function checkAllItemBelow(identifier) {
    ...
    function checkSingle(identifier) {
