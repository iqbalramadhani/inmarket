...
class ComplaintProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($complain->isEmpty()) {
            ...
            $notif = NotificationUtility::sendNotification($order, 'complained');
    ...
    public function storeOrder(Request $request)
    {
        ...
        if ($complains->isEmpty()) {
            $complain = Complain::create([
                'order_id' => $order->id,
                'order_detail_id' => $order->orderDetails()->first()->id,
            ...
            if ($conversation->save()) {
                ...
                $message->message = "Halo, Saya anda kendala dengan pembelian produk ini";
    ...
    public function store_delivery($id, Request $request)
    {
        ...
        $complain->resi_courier_service = $request->resi_courier_service ?? null;
    ...
    public function complete($id, Request $request)
    {
        ...
        if ($complain->resi_number && $complain->status == 'procceeded') {
            ...
            //     if (env('APP_ENV')=='production' && $result['rajaongkir']['status']['code'] == 200) {
                    $amount = $complain->order()->first()->orderDetails()->sum('price') + $complain->order()->first()->orderDetails()->sum('total_commission_inatrade');
