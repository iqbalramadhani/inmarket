...
class CheckoutController extends Controller
{
            ...
            if($vendor_commission_activation){
                foreach ($order->orderDetails as $key => $orderDetail) {
                    ...
                    if ($orderDetail->product->user->user_type == 'seller') {
                        ...
                        $admin_commission = $orderDetail->total_commission_inatrade;
    ...
    public function checkout_done_2($order_id, $payment)
    {
        ...
        if($vendor_commission_activation){
            foreach ($order->orderDetails as $key => $orderDetail) {
                ...
                if ($orderDetail->product->user->user_type == 'seller') {
                    $seller = $orderDetail->product->user->seller;
                    $real_price = (100/100 + $commission_percentage) * $orderDetail->price;
                    $admin_commission = $orderDetail->price - $real_price;
    ...
    public function store_shipping_info(Request $request)
    {
        ...
        $carts = Cart::with('product')->where('user_id', Auth::user()->id)->where('selected_to_buy', true)->get();
        $owners = array_keys($carts->groupBy('owner_id')->toArray());
        ...
        $data_shipping = [
            ...
            'subdistrict_id_from' => $subdistrict_id_from,
            'subdistrict_id_to' => $subdistrict_id_to
        ];
    ...
    public function store_delivery_info(Request $request)
    {
        ...
        $carts_all = Cart::where('user_id', Auth::user()->id)
                 ->where('selected_to_buy', true)
                ->get();



        if($carts_all->isEmpty()) {
            flash(translate('Your cart is empty'))->warning();
            return redirect()->route('home');
        }

        // ambil alamat pengiriman
        $shipping_info = Address::where('id', $carts_all[0]['address_id'])->first();
        ...
        foreach($request->owner_ids as $owner_id){
            $carts = $carts_all->where('owner_id', $owner_id);
            $counter_for_input_cost_shipping = 0;
            ...
            if ($carts && count($carts) > 0) {
                foreach ($carts as $key => $cartItem) {
                    ...
                    // dan apakah value dari key shipping cost berbentuk array?
                    ...
                    if($counter_for_input_cost_shipping === 0){
                        // menyimpan cost hanya sekali kalau dalam satu toko produk yg dibeli lebih dari satu
                        $cartItem['shipping_cost'] = json_decode($request->service_type[$owner_id])->cost[0]->value;
                    }
                    $counter_for_input_cost_shipping++;
